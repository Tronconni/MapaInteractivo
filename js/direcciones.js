direccionesModulo = (function() {
	...
	function calcularYMostrarRutas() {
		// hago un switch para obtener el value ingresado
		let comoIrResultado = '';
		const medioTransporte = document.getElementById('comoIr').value;
		switch (medioTransporte) {
			case 'Caminando':
				comoIrResultado = 'WALKING';
				break;
			case 'Bus/Subterraneo/Tren':
				comoIrResultado = 'TRANSIT';
				break;
			case 'Bicicleta':
				comoIrResultado = 'BICYCLING';
				break;
			default:
				comoIrResultado = 'DRIVING';
				break;
		}

		let getWayPoint = [ ...document.getElementById('puntosIntermedios') ];
		for (let i = 0; i < getWayPoint.length; i++) {
			waypts.push({ location: getWayPoint[i].value, stopover: true });
		}
		let direccionReq = {
			origin: document.getElementById('desde').value,
			destination: document.getElementById('hasta').value,
			// travelMode: google.maps.DirectionsTravelMode.DRIVING,
			travelMode: comoIrResultado,
			unitSystem: google.maps.UnitSystem.METRIC
		};
		servicioDirecciones.route(direccionReq, (response, status) => {
